...
<div class="p-0 md:p-4">
    <div class="px-4 py-8 md:px-0 md:py-4 max-w-4xl mx-auto space-y-8">
        <div class="flex items-center justify-between" data-aos="fade-down">
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900 font-display tracking-tight mb-1">Tambah Produk Hukum</h1>
                <p class="text-slate-500">Input data peraturan atau keputusan resmi desa.</p>
            </div>
    ...
    <form action="{{ route('admin.legal-products.store') }}" method="POST" class="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-xl shadow-slate-200/50 border border-slate-100 space-y-8" data-aos="fade-up">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" required placeholder="Contoh: Peraturan Desa No. 1 Tahun 2024 tentang APBDes"
        ...
        <div>
            ...
            <textarea name="description" rows="4" placeholder="Jelaskan isi singkat dari produk hukum ini..."
                      class="w-full px-6 py-4 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all shadow-sm leading-relaxed"></textarea>
        </div>
        ...
        <div class="pt-6">
            <button type="submit" class="w-full md:w-auto px-12 py-5 bg-emerald-600 text-white font-bold rounded-2xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 hover:-translate-y-1 transition-all flex items-center justify-center">
                <i class="bi bi-check-circle-fill mr-3"></i> Simpan Produk Hukum
            </button>
        </div>
    </form>
    </div>
