...
<head>
    ...
    <title>@yield('title', 'Aparatur Panel') | {{ $villageName }}</title>
...
<body class="bg-gray-50 text-slate-800 antialiased" x-data="{ mobileMenu: false }">
    ...
    <div class="lg:hidden bg-slate-900 text-white p-4 flex justify-between items-center sticky top-0 z-[60]">
        <div class="flex items-center space-x-3">
            ...
            @endif
            <span class="font-bold text-sm tracking-tight text-blue-400">Panel Aparatur</span>
        </div>
    ...
    <aside :class="mobileMenu ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'" 
        ...
        <div class="p-8 hidden lg:block">
            <div class="flex items-center space-x-3 mb-2">
                ...
                @endif
                <div>
                    <h1 class="text-white text-lg font-black tracking-tight leading-none uppercase">Apa<span class="text-blue-500">ratur.</span></h1>
        ...
        <nav class="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto min-h-0 custom-scrollbar">
            ...
            <a href="{{ route('admin.dashboard') }}" 
               class="flex items-center px-4 py-3 rounded-2xl transition-all group {{ Request::is('admin') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                <i class="bi bi-grid-fill mr-4 text-xl {{ Request::is('admin') ? 'text-white' : 'text-slate-500 group-hover:text-blue-400' }}"></i>
            ...
            <a href="{{ route('agendas.index') }}" 
               class="flex items-center px-4 py-3 rounded-2xl transition-all group {{ Request::is('admin/agendas*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                <i class="bi bi-calendar-event-fill mr-4 text-xl {{ Request::is('admin/agendas*') ? 'text-white' : 'text-slate-500 group-hover:text-blue-400' }}"></i>
                <span class="font-bold text-sm tracking-wide">Agenda {{ $villageTerm }}</span>
            </a>

            <a href="{{ route('admin.news.index') }}" 
               class="flex items-center px-4 py-3 rounded-2xl transition-all group {{ Request::is('admin/news*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                <i class="bi bi-newspaper mr-4 text-xl {{ Request::is('admin/news*') ? 'text-white' : 'text-slate-500 group-hover:text-blue-400' }}"></i>
                <span class="font-bold text-sm tracking-wide">Berita {{ $villageTerm }}</span>
            </a>

            <a href="{{ route('admin.marketplace.index') }}" 
               class="flex items-center px-4 py-3 rounded-2xl transition-all group {{ Request::is('admin/marketplace*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'hover:bg-slate-800 hover:text-white' }}">
                <i class="bi bi-shop mr-4 text-xl {{ Request::is('admin/marketplace*') ? 'text-white' : 'text-slate-500 group-hover:text-blue-400' }}"></i>
        ...
        <header class="h-20 bg-white border-b border-slate-200 sticky top-0 z-[50] flex items-center justify-between px-8 hidden lg:flex shadow-sm">
            <div>
                <h2 class="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center">
                    Panel Aparatur <i class="bi bi-chevron-right mx-3 opacity-50"></i> <span class="text-slate-900">@yield('title', 'Ringkasan')</span>
                </h2>
            </div>
            
            <div class="flex items-center space-x-6">
                <!-- Live Preview Link -->
                <a href="/" target="_blank" class="flex items-center text-xs font-black text-blue-600 uppercase tracking-widest hover:text-blue-700 transition">
                ...
                <div class="flex items-center space-x-4">
                    <div class="text-right">
                        ...
                        <p class="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-1">Aparatur</p>
                    </div>
                    <div class="w-10 h-10 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-center text-blue-600 font-bold overflow-hidden shadow-sm">
